...
Decrease the font-size of vterm buffers using the text-scale so it's based off
the =doom-font= setting.
...
** Language Server Protocol :lsp:ui:
...
NOTE: The format module is currently on the do-not-bump list due to incompatible
upstream changes. Going to disable format all on ts and js files in favor of
prettier.el
...
*** Prettier
Docs at https://github.com/jscheid/prettier.el
...
Enable global mode to format on save:

#+begin_src emacs-lisp
(add-hook! '(json-mode typescript-tsx-mode typescript-mode rjsx-mode js2-mode js-mode) #'prettier-mode)
#+end_src

Alternatively, could use dir-locals to enable prettier mode by project
https://github.com/jscheid/prettier.el#enabling-per-file--per-directory

Configure enabled langs prettier is intended to format. Requires respective
prettier plugins globally installed via yarn or npm:
...
Use the TAB key for autocomplete. Support cycling through completions with TAB if there are only a few candidates.
...
** Simplify centaur-tabs buffer groups :ui:centaur:tabs:

Currently centaur will only display buffers of the same file type in the tabs
UI. However, I prefer it displays all open tabs of the project so it's easier to
see what's actually opened.
...
** Janet Language :lang:lisp:janet:

Add packages for the Janet lisp lang. Packages were linked from https://github.com/janet-lang/janet.
